                  ...
                  <div class="row center">
                        ...
                        <div class="container">
                           <div class="form-row">
                              <div class="col-md-4 profile-pr">
                                 <div class=" {{ $errors->has('phone_no') ? 'has-error' : '' }}">
                     ...
                     <div class="profile-text">
                        ...
                        <div class="container">
                           <div class="form-row">
                              <div class="col-md-4 profile-pr">
                                 <div class=" {{ $errors->has('house_no') ? 'has-error' : '' }}">
                              ...
                              <div class="col-md-4 yr ptop  profile-pr">
                                 ...
                                 <div class="sel">
                                    ...
                                    <select name="country" class="country-name" id="selectbox">
                                    @if(!empty($profile->country))
                                    <option value="{{ $profile->country }}">{{ $select->selectedCounntry($profile->country)[0]->name }}</option>
                                    @else
                              ...
                              <div class="col-md-4 yr ptop profile-pr">
                                 ...
                                 <div class="sel">
                                    <select name="user_state" class="state-name" id="selectbox">
                                    @if(!empty($profile->state))
                                    <option value="{{ $profile->state }}">{{$select->selectedState($profile->state)[0]->name}}</option>
                                    @else
                              ...
                              <div class="col-md-4 yr ptop profile-pr">
                                 ...
                                 <div class="sel">
                                    <select name="user_city" class="city-name" id="selectbox">
                                    @if(!empty($profile->city))
                                    <option value="{{ $profile->city }}">{{ $select->selectedCity($profile->city)[0]->name }}</option>
                                    @else
                        ...
                        <div class="container">
                           <div class="form-row">
                              <div class="col-md-4 yr profile-pr">
                                 ...
                                 <span class="text-danger">{{ $errors->first('user_ext_no') }}</span>
                              </div>
                              @if( $user->type_id == $user::TYPE_ID_DOCTOR )
                              <div class="col-md-4 yr profile-pr">
                                 <label for="phone">Specialist</label>
                                 <div class="sel">
                                    <select name="specialist" id="selectbox">
									@if(!empty($profile->specialist))
										@if(!empty($profile->specialist))
											wdwefl;elf
											<option value="{{ $profile->specialist }}">{{ $profile->specialist }}</option>
											<option disabled></option>
										@endif
                                       @elase(!empty($categories))
...
<script>
   $(".country-name").change(function(){
      ...
      var url = "{{ url('state') }}/"+country_name;
      $.ajax({
         url : url,
         type: 'GET',
            success: function(res){
                $('.state-name').find("option").remove();
                $('.state-name').append(res);
            }
      });
   });
</script>
...
<script>
   $(".state-name").change(function(){
      ...
      var url = "{{ url('city') }}/"+city_id;
      $.ajax({
         url : url,
         type: 'GET',
            success: function(res){
                console.log(res);
                $(".city-name").find("option").remove();
                $(".city-name").append(res);
            }
      });
   });
</script>
